...
Como parte desta solução, **toda string estática no framework de Rails** - por exemplo, validações de mensagens no Active Record, formatos de hora e data - **foram internacionalizados**. _Localization_ de uma aplicação Rails significa definir valores traduzidos para essas strings nos idiomas desejados.
