...
WARNING: Use os métodos de configuração pública em vez de chamar diretamente para a classe associada. por exemplo. `Rails.application.config.action_mailer.options` em vez de `ActionMailer::Base.options`.

NOTE: Se você precisar aplicar a configuração diretamente a uma classe, use um [lazy load hook](https://api.rubyonrails.org/classes/ActiveSupport/LazyLoadHooks.html) em um *initializer* para evitar o carregamento automático da classe antes da inicialização concluído. Isso será interrompido porque o carregamento automático durante a inicialização não pode ser repetido com segurança quando o aplicativo for recarregado.
...
Os métodos de configuração a seguir devem ser enviados para objetos do tipo
`Rails::Railtie`, como uma subclasse de `Rails::Engine` ou
`Rails::Application`.

#### `config.after_initialize`

Recebe um bloco que será executado _após_ a inicialização da aplicação Rails. Isso inclui a inicialização do *framework* em si, *engines*  e todos *initializers*  definidos em
...
Define o *host* para os *assets*. Essa opção é útil quando
...
Aceita um *array* de caminhos de onde o Rails
...
Aceita um *array* de caminhos de onde o Rails deve
carregar automaticamente constantes. O padrão é um array vazio. Desde o [Rails 6](upgrading_ruby_on_rails.html#autoloading),
não é mais recomendado ajustar essa configuração. Veja mais em [Auto carregamento e Recarregando Constantes](autoloading_and_reloading_constants.html#autoload-paths-and-eager-load-paths)

#### `config.add_autoload_paths_to_load_path`

Determina se os caminhos de auto carregamento devem ser adicionados para `$LOAD_PATH`. Esta configuração é `true` por padrão, mas é recomendado que seja
...
Controla quando as classes e módulos da aplicação devem ser recarregados em caso de mudanças. Quando o cache está ativado (`true`) o recarregamento não irá acontecer.
...
Configura o começo da semana para a aplicação. O
...
ou um objeto que implementa a API de cache. O padrão é `:file_store`. Veja [Cache Stores](caching_with_rails.html#cache-stores) para opções de configuração por *store*.

#### `config.colorize_logging`

Determina quando utilizar ou não códigos de cores ANSI ao registrar informações no *log*. O padrão é `true`.

#### `config.consider_all_requests_local`

Quando `true` adiciona
...
defina como `false` e implemente o método `show_detailed_exceptions?` em seus *controllers*
para especificar que requisições devem prover informações de *debug* em erros.

#### `config.console`

Permite que você defina qual classe deve ser usada quando executar `bin/rails console`. É melhor que seja executada dentre de um bloco `console`: 
...
Controla se o *console* pode ser iniciado ou não no
...
Quando `true`, todos *namespaces* definidos em `config.eager_load_namespaces` são carregados de forma ativa (*eager load*).
Isto inclui a sua aplicação, *engines*, frameworks do Rails e qualquer outra
*namespace* definida.

#### `config.eager_load_namespaces`

Define todas *namespaces* que serão carregadas ativamente (*eager load*) quando `config.eager_load` estiver definido como `true`.
Todas *namespaces* na lista devem responder ao método `eager_load!`.

#### `config.eager_load_paths`

Aceita um *array* de caminhos de onde o Rails deve carregar ativamente durante a inicialização se o cache de classes está ativo.
O padrão são todas as pastas dentro do diretório `app`.

#### `config.enable_dependency_loading`

Quando definido como `true`, habilita o autocarregamento mesmo que a aplicação esteja carregada ativamente (*eager load*)
e que `config.cache_classes` esteja definido como `true`. O padrão é `false`

#### `config.encoding`

Define o *encoding* para toda a aplicação. O padrão é UTF-8.

#### `config.exceptions_app`

Define a aplicação invocada pelo *middleware*
`ShowException` quando uma exceção acontece. O padrão é `ActionDispatch::PublicExceptions.new(Rails.public_path)`.

#### `config.debug_exception_response_format`

Define o formato usado nas respostas quando errors acontecem em modo desenvolvimento.
O padrão é `:api` para aplicações somente API e `:default` para aplicações
regulares.

#### `config.file_watcher`

É a classe usada para detectar atualizações nos arquivos
...
Usada para filtrar os parâmetros que você não quer que apareçam nos logs,
como senhas ou números de cartão de crédito. Também filtra valores sensíveis
de colunas do banco de dados ao chamar `#inspect` em um objeto Active Record. Por
padrão, o Rails filtra as senhas adicionando os seguintes filtros em
...
O filtro de parâmetros funciona por correspondência com expressões regulares parciais.

#### `config.force_ssl`

Impõe que todas requisições sejam tratadas através de HTTPS e define "https://" como protocolo padrão para gerar URLs. Execução de HTTPS é manipulada pelo *middleware* `ActionDispatch::SSL`, que pode ser configurado através de `config.ssl_options`.

#### `config.javascript_path`

Define o caminho onde o JavaScript da sua aplicação
reside em relação ao diretório `app`. O padrão é `javascript`, usado pelo
[webpacker](https://github.com/rails/webpacker). O `javascript_path` configurado
de uma aplicação será excluído de `autoload_paths`.

#### `config.log_formatter`

Define o formatador do *logger* do Rails. O valor
padrão é uma instância de `ActiveSupport::Logger::SimpleFormatter` para todos os ambientes.
...
Define a verbosidade do *logger* do Rails. O padrão dessa
opção é `:debug` para todos ambientes exceto produção, onde o padrão é `:info`. Os níveis disponíveis são `:debug`, `:info`, `:warn`, `:error`, `:fatal`, e `:unknown`.

#### `config.log_tags`

Aceita uma lista de: mmétodos que o objeto `request`
...
É o *logger* que será usado pelo `Rails.logger` e qualquer outro *log* relacionado ao Rails, tal como `ActiveRecord::Base.logger`. 
...
Permite configurar o *middleware* da aplicação. Esse tópico é coberto em detalhes em [Configurando o Middleware](#configuring-middleware).

#### `config.rake_eager_load`

Quando` true`, carrega antecipadamente a aplicação ao executar tarefas Rake. O padrão é `false`.

#### `config.reload_classes_only_on_change`

Habilita ou desabilita a recarga de classes somente quando arquivos monitorados são modificados. Por padrão são
...
Configura o caminho para busca por credenciais criptografadas.

#### `config.credentials.key_path`

Configura o caminho para busca por chaves de criptografia.

#### `secret_key_base`

É usado para especificar uma chave que permite que sessões da aplicação sejam verificadas contra uma chave segura conhecida prevenindo falsificações (*tampering*). Aplicações recebem uma chave gerada
...
Faz com que a app não inicialize se uma chave mestre (_master key_) não tiver sido disponibilizada através de `ENV["RAILS_MASTER_KEY"]` ou do arquivo `config/master.key`.

#### `config.public_file_server.enabled`

Configura o Rails para servir arquivos estáticos a partir do diretório `public`.
...
Define que classe usar para armazenar a sessão. Os valores possíveis são `:cookie_store`, `:mem_cache_store`, algum valor customizado,
ou `:disabled`. `:disabled` indica que o Rails não deve tratar de sessões.

Essa configuração é definida por meio de uma chamada de método regular, em vez de um *setter*. Isso permite que opções adicionais sejam passadas:

```ruby
config.session_store :cookie_store, key: "_your_app_session"
```

Se um armazenamento personalizado for especificado como um *symbol*, ele será resolvido para o *namespace* `ActionDispatch::Session`:
...
O armazenamento padrão é um armazenamento de cookies com o nome da aplicação como chave de sessão.

#### `config.ssl_options`

Opções de configuração para o *middleware* [`ActionDispatch::SSL`](https://api.rubyonrails.org/classes/ActionDispatch/SSL.html).

O valor padrão depende da versão configurada em `config.load_defaults`:

| Começando com versão  |   O valor padrão é   |
| --------------------- | -------------------- |
| (original)            | `{}`                 |
...
Define o fuso horário para a aplicação e torna o *Active Record* ciente deste fuso horário.
