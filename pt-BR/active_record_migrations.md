...
[alterar nosso esquema de banco de dados com o tempo](https://en.wikipedia.org/wiki/Schema_migration) de uma forma consistente. Elas usam uma *Ruby DSL* para que você não precise escrever SQL puro, permitindo que seu esquema e as alterações sejam independentes do banco de dados utilizado.
...
irá gerar o `add_column` e [`add_index`][]:
...
gera seguinte chamada a [`add_references`][]:
...
* [`add_reference`][]
* [`add_timestamps`][]
* [`change_column_default`][] (deve ser fornecida as opções :from e :to)
* [`change_column_null`][]
* [`create_join_table`][]
* [`create_table`][]
* `disable_extension`
* [`drop_join_table`][]
* [`drop_table`][] (deve ser fornecido um bloco)
* `enable_extension`
* [`remove_column`][] (deve ser fornecido um tipo)
* [`remove_foreign_key`][] (deve ser fornecido uma segunda tabela)
* [`remove_index`][]
* [`remove_reference`][]
* [`remove_timestamps`][]
* [`rename_column`][]
...
Para inserir dados depois que um banco de dados for criado, o Rails tem uma funcionalidade padrão de *seeds* que aceleram o processo. Isto é especialmente útil ao recarregar o banco de dados com frequência nos ambientes de teste e desenvolvimento. Para começar a usar esta funcionalidade, preencha o arquivo `db/seeds.rb` com código Ruby, e execute `bin/rails db:seed`:
