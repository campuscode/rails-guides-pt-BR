...
O método [`pluralize`][String#pluralize] retorna o plural do seu receptor:
...
Como mostra o exemplo anterior, Active Support conhece alguns plurais irregulares e substantivos incontáveis. As regras integradas podem ser estendidas em `config/initializers/inflections.rb`. Este arquivo é gerado por padrão, pelo `rails new` comando e tem instruções nos comentários.

`pluralize` também pode fazer um opcional `count` parâmetro. Se `count == 1` a forma singular será retornada. Para qualquer outro valor de `count` a forma plural será retornada:
...
Active Record usa este método para calcular o nome da tabela padrão que corresponde a um modelo:
...
As associações calculam o nome da classe associada padrão correspondente usando este método:
...
O método [`camelize`][String#camelize] retorna seu receptor em caixa de camelo:
...
Como regra geral, você pode pensar neste método como aquele que transforma caminhos em classes Ruby ou nomes de módulo, onde barras separam namespaces:
...
`camelize` aceita um argumento opcional, pode ser `:upper` (default), or `:lower`. Com o último, a primeira letra torna-se minúscula:
...
INFO: Como regra geral, você pode pensar em `camelize` como o inverso de `underscore`, embora haja casos em que isso não se aplica: `"SSLError".underscore.camelize` devolve`"SslError"`. Para apoiar casos como este, Active Support permite que você especifique acrônimos em `config/initializers/inflections.rb`:
...
O método [`underscore`][String#underscore] vai ao contrário, de caixa de camelo a caminhos:
...
`underscore` não aceita nenhum argumento embora.

Classe Rails e uso de carregamento automático de módulo `underscore` para inferir o caminho relativo sem extensão de um arquivo que definiria uma determinada constante ausente:
...
INFO: Como regra geral, você pode pensar em `underscore` como o inverso de `camelize`, though, há casos em que isso não acontece. For example, `"SSLError".underscore.camelize` devolve `"SslError"`.
...
O método [`titleize`][String#titleize] capitaliza as palavras no receptor:
...
`titleize` tem o alias de [`titlecase`][String#titlecase].
...
O método [`dasherize`][String#dasherize] substitui os sublinhados no receptor por travessões:
...
O serializador XML de modelos usa este método para dasherize nomes de nós:
...
Dado uma string com um nome de constante qualificado, [`demodulize`][String#demodulize]retorna o próprio nome constante, ou seja, a parte mais à direita dele:
...
O Active Record, por exemplo, usa este método para calcular o nome de uma coluna de cache do contador:
...
Dada uma string com uma expressão de referência constante qualificada, [`deconstantize`][String#deconstantize] remove o segmento mais à direita, geralmente deixando o nome do contêiner da constante:
...
O método [`parameterize`][String#parameterize] normaliza seu receptor de uma forma que pode ser usada em URLs bonitos.
...
Para preservar a caixa da corda, defina o argumento `preserve_case` para true. Por padrão, `preserve_case` está definido como falso.
...
Para usar um separador personalizado, sobrescrever o argumento `separador`.
...
O método [`tableize`][String#tableize] é `sublinhado` seguido por` pluralize`.
...
Como um princípio básico, `tableize` retorna o nome da tabela que corresponde a um determinado modelo para casos simples. A implementação real no Active Record não é diretamente `tableize`, porque também demoduliza o nome da classe e verifica algumas opções que podem afetar a string retornada.
...
O método [`classify`][String#classify] é o inverso de `tableize`. Dá-lhe o nome da classe correspondente ao nome da tabela:
...
Observe que `classify` retorna um nome de classe como uma string. Você pode obter o objeto de classe real invocando `constantize` nele, explicado a seguir.
...
Se a string for avaliada como nenhuma constante conhecida, ou seu conteúdo nem mesmo for um nome de constante válido, `constantize` gerará` NameError`.

A resolução de nome constante por `constantize` começa sempre no` Object` de nível superior, mesmo se não houver nenhum "::" inicial.
...
Então, em geral não é equivalente ao que Ruby faria no mesmo local, se uma constante real fosse avaliada.

Os casos de teste do Mailer obtêm o mailer que está sendo testado a partir do nome da classe de teste usando `constantize`:
...
O método [`humanize`][String#humanize] ajusta um nome de atributo para exibir aos usuários finais.

Especificamente, ele realiza essas transformações:

  * Aplica regras de inflexão humana ao argumento.
  * Exclui sublinhados à esquerda, se houver.
  * Remove um sufixo "_id" se presente.
  * Substitui sublinhados por espaços, se houver.
  * Reduz todas as palavras, exceto acrônimos.
  * Capitaliza a primeira palavra.

A capitalização da primeira palavra pode ser desativada configurando opção `: capitalize` para falso (o padrão é verdadeiro).
...
Se "SSL" foi definido como uma sigla:

```ruby
'ssl_error'.humanize # => "SSL error"
```

O método auxiliar `full_messages` usa` humanize` como um substituto para incluir
nomes de atributos:
...
O método [`foreign_key`][String#foreign_key] fornece um nome de coluna de chave estrangeira a partir de um nome de classe. Para fazer isso, ele desmoduliza, sublinha e adiciona "_id":
...
Associações usam este método para inferir chaves estrangeiras, por exemplo `has_one` e` has_many` fazem isso:
...
Os métodos [`to_date`][String#to_date], [`to_time`][String#to_time], and [`to_datetime`][String#to_datetime] são basicamente invólucros de conveniência `Date._parse`:
...
`to_time` recebe um argumento opcional `:utc` or `:local`, para indicar em qual fuso horário você deseja que o horário:
